...
@testset "Mock optimizer automatic constraint primal" begin
    ...
    @testset "Conic SOC tests" begin
        optimizer.optimize! = (optimizer::MOIU.MockOptimizer) -> mock_optimize!(optimizer, [1.0, 1/√2, 1/√2], (MOI.VectorOfVariables,             MOI.SecondOrderCone) => [[√2, -1, -1]],
                                                                                                              (MOI.VectorAffineFunction{Float64}, MOI.Zeros)           => [[-√2]])
        MOIT.soc1vtest(optimizer, config)
        optimizer.optimize! = (optimizer::MOIU.MockOptimizer) -> mock_optimize!(optimizer, [1.0, 1/√2, 1/√2], (MOI.VectorAffineFunction{Float64}, MOI.SecondOrderCone) => [[√2, -1, -1]],
                                                                                                              (MOI.VectorAffineFunction{Float64}, MOI.Zeros)           => [[-√2]])
        MOIT.soc1ftest(optimizer, config)
        optimizer.optimize! = (optimizer::MOIU.MockOptimizer) -> mock_optimize!(optimizer, [-1/√2, 1/√2, 1.], (MOI.VectorAffineFunction{Float64}, MOI.SecondOrderCone) => [[√2, 1, -1]],
                                                                                                              (MOI.VectorAffineFunction{Float64}, MOI.Zeros)           => [[√2]],
