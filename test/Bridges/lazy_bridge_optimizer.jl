...
Bridge graph with 8 variable nodes, 17 constraint nodes and 0 objective nodes.
 ...
 [5] constrained variables in `MOI.Scaled{MOI.PositiveSemidefiniteConeTriangle}` are not supported
 ...
 (10) `MOI.VectorAffineFunction{$T}`-in-`MOI.Scaled{MOI.PositiveSemidefiniteConeTriangle}` constraints are not supported
 (11) `MOI.VectorOfVariables`-in-`MOI.Scaled{MOI.PositiveSemidefiniteConeTriangle}` constraints are not supported
