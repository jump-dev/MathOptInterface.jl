...
@testset "loadfromstring" begin
    ...
    @testset "linear constraints" begin
        ...
        linear1 = MOI.add_constraint(model, MOI.ScalarAffineFunction(MOI.ScalarAffineTerm.([1.0, 1.0], [x, y]), 0.0), MOI.GreaterThan(1.0))
        linear2 = MOI.add_constraint(model, MOI.ScalarAffineFunction(MOI.ScalarAffineTerm.([1.0, 1.0], [x, y]), 0.0), MOI.LessThan(1.0))
        linear3 = MOI.add_constraint(model, MOI.ScalarAffineFunction(MOI.ScalarAffineTerm.([1.0, 1.0], [x, y]), 0.0), MOI.EqualTo(1.0))
    ...
    @testset "SOC constraints" begin
        ...
        varsoc = MOI.add_constraint(model, MOI.VectorOfVariables([x,y,z]), MOI.SecondOrderCone(3))
        affsoc = MOI.add_constraint(model, MOI.VectorAffineFunction(MOI.VectorAffineTerm.([1,2,3], MOI.ScalarAffineTerm.([2.0,1.0,-1.0], [x,y,z])), [0.0,1.0,0.0]), MOI.SecondOrderCone(3))
        affsoc2 = MOI.add_constraint(model, MOI.VectorAffineFunction(MOI.VectorAffineTerm{Float64}[],[1.0,2.0,3.0]), MOI.SecondOrderCone(3))
