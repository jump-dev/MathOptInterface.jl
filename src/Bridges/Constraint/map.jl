...
function _ensure_available(
    ...
    # `length(map.bridges) + 1` would be the next slot allocated; advance
    # past any slot whose `CI{F, S}` value is already in use by another
    # bridge layer (typically `Variable.bridges`, which holds the bridged
    # `CI{VOV, S}` of constrained-on-creation vectors of variables).
