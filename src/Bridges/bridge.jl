...
"""
    cost(BT::Type{<:AbstractBridge})::Float64

Return the *cost* a bridge of type `BT`. This cost is used to compute the
[`bridging_cost`](@ref). There is a default implementation for any
[`AbstractBridge`](@ref) returning `1.0` so this should only be implemented
for bridges returning a distance different from `1.0`.
...
For instance, we want to prioritize bridging a
...
over bridging it to a
[`MOI.ScalarQuadraticFunction`](@ref)-in-[`MOI.LessThan`](@ref) constraint.
