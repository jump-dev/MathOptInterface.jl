...
Base.length(it::ScalarFunctionIterator{<:MOI.AbstractVectorFunction}) = MOI.output_dimension(it.f)
...
_rmvar(vis_or_terms::Vector, vi::MOI.VariableIndex) = vis_or_terms[.!_hasvar.(vis_or_terms, Ref(vi))]
