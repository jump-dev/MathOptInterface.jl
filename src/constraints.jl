...
* ``q_i(x)`` is a vector with component ``(q_i(x))_k`` defined as ``\frac{1}{2} x^T Q_{i,k} x``, where each symmetric matrix ``Q_{i,k}`` has ``Q_constridx = k`` and is specified in triplet form by ``Q_vari, Q_varj, Q_coef``
...
Return the element ``(v_1, v_2)`` of the ``Q_{i,k}`` matrix of the ``k``th row of the constraint corresponding to ``c``.
"""
