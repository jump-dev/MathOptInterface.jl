...
Parse `obj` into a [`NonlinearExpression`](@ref) and set as the objective
function of `data`.
...
Parse `expr` into a [`NonlinearExpression`](@ref) and add to `data`. Returns an
...
Parse `input` into a [`NonlinearConstraint`](@ref) and add to `data`. Returns a
