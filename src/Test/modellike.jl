...
function copytest(dest::MOI.ModelLike, src::MOI.ModelLike)
    ...
    cva = MOI.addconstraint!(src, MOI.VectorAffineFunction(MOI.VectorAffineTerm.([1, 2], MOI.ScalarAffineTerm.(ones(2), [v[3], v[2]])), [-3.0,-2.0]), MOI.Zeros(2))
    ...
    @test MOI.get(dest, MOI.ConstraintFunction(), dict[cva]) ≈ MOI.VectorAffineFunction(MOI.VectorAffineTerm.([1, 2], MOI.ScalarAffineTerm.(ones(2), [dict[v[3]], dict[v[2]]])), [-3.0,-2.0])
