...
Recall the discussion on references in [`isvalid`](@ref MathOptInterface.isvalid). A simple way assign integer values to references to avoid unintentional conflicts with other instances is to use `xor`, an invertible operation, with an arbitrary integer value like `object_id` as the key. `xor` can be used again to covert back to the internal numbering scheme. This is not required but is a small trick that will most likely quickly catch a dangerous class of bugs.
