...
function _automatic_reference_documentation(
    ...
    config = _Config(current_module, root, subdirectory, modules, exclude)
...
function _exported_symbols(config::_Config)
    contents = Pair{Symbol,DocType}[]
    for n in names(config.current_module; all = true)
        f = getfield(config.current_module, n)
        if f in config.exclude
            continue
        end
        f_str = string(f)
        if startswith("$n", "_") || startswith("$n", "#")
            continue
        elseif n in (:eval, :include)
            continue
        elseif startswith(f_str, "@")
...
function _iterate_over_symbols(f, config)
    ...
    for (key, type) in vcat(_exported_symbols(config), modules)
        if key isa Symbol
            ...
            if occursin("No documentation found.", string(doc))
                ...
                elseif type == DOCTYPE_CONSTANT
                    ...
                    @warn("Documentation missing for $key")
                    continue
...
function _build_api_page(document::Documenter.Document, config::_Config)
    ...
    ```@meta
    EditURL = nothing
    CurrentModule = $(config.current_module)
    ```
